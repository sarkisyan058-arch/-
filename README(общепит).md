...
# **Проверим типы данных и оценим их корректность:**
# 
# Типы данных корректны и не нуждаются в преобразовании на данном этапе.

# **Другие особенности**
# 
# - На этом этапе можно обратить внимание на:
# 
# <span style="color:#4682B4">Нестандартные форматы в столбце **hours** (например, «круглосуточно», «пн‑пт 9:00–22:00»).</span>.
# 
# <span style="color:#4682B4">Разнообразие категорий в **category** и **price** (возможны опечатки, синонимы).</span>.
# 
# <span style="color:#4682B4">Разброс значений в **rating** (должен быть от 0 до 5) и **seats** (возможны аномально высокие или нулевые значения).</span>.
...
# 
# <span style="color:#4682B4">Все столбцы присутствуют</span>.
# 
# <span style="color:#4682B4">Названия соответствуют спецификации</span>.

# **Пропущенные значения**:
# 
# <span style="color:#4682B4">В ***rest_info*** пропуски обнаружены в столбцах: (**hours** - 536 и в **seats** - 3611 ).
# 
# <span style="color:#4682B4">В ***rest_price*** пропуски есть в: (**price** - 743, **avg_bill** - 242, **middle_avg_bill** - 909, **middle_coffee_cup** - 3523).</span>

# **Типы данных:**
# 
# <span style="color:#4682B4">Числовые столбцы (**rating**, **seats**, **middle_avg_bill**) имеют тип **float64**.
# 
# <span style="color:#4682B4"><span style="color:#4682B4">Категориальные (**category**, **price**) — тип **object**.
# 
# <span style="color:#4682B4">При необходимости потребуется преобразование (например, **chain** в **bool**).</span>
# 

# **Особенности данных:**
# 
# <span style="color:#4682B4">Столбец **hours** содержит неструктурированные текстовые данные</span>.
# 
# <span style="color:#4682B4">В **avg_bill** присутствуют разные форматы цен (диапазоны, отдельные значения, упоминания капучино/пива)</span>.
# 
# <span style="color:#4682B4">Возможны дубликаты по **name*** или **address**</span>.
# 
# <span style="color:#4682B4">Аномалии в **seats** (нулевые или экстремально высокие значения)</span>.
...
# **Анализируем каждый столбец с пропусками и выдвигаем гипотезы о причинах их появления.**

# 1. hours (часы работы)
# 
# <span style="color:#4682B4">Количество пропусков: 535 (6.37%)
# 
# <span style="color:#4682B4">Гипотеза: данные могли не быть заполнены владельцами заведений или не найдены в открытых источниках. <span style="color:#4682B4">Возможно, некоторые заведения не указали график работы намеренно (например, работают по записи).
# 
# 
# 2. seats (количество посадочных мест)
# 
# <span style="color:#4682B4">Количество пропусков: 3610 (42.97%)
# 
# <span style="color:#4682B4">Гипотеза: владельцы не указали количество мест; заведение имеет гибкую рассадку (например, <span style="color:#4682B4">фуд‑корт); 
# <span style="color:#4682B4">данные сложно оценить точно для больших пространств.
# 
# 3. middle_avg_bill (средний чек)
# 
# <span style="color:#4682B4">Количество пропусков: 5253 (62.52%)
# 
# <span style="color:#4682B4">Гипотеза: в столбце avg_bill нет информации о «Среднем счёте»; 
# <span style="color:#4682B4">данные отсутствуют в источнике;
# <span style="color:#4682B4">заведение не указало ценовой диапазон.
# 
# 5. middle_coffee_cup (цена чашки капучино)
# 
# <span style="color:#4682B4">Количество пропусков: 7867 (93.63%)
# 
# <span style="color:#4682B4">Гипотеза: заведение не продаёт кофе;
# <span style="color:#4682B4">в avg_bill нет упоминания цены капучино;
# <span style="color:#4682B4">данные не собраны.
# 
# 6. price (категория цен)
# 
# <span style="color:#4682B4">Количество пропусков: 5087 (60.55%)
# 
# <span style="color:#4682B4">Гипотеза: категория цен не была указана при регистрации заведения;
# <span style="color:#4682B4">данные не удалось автоматически классифицировать.
# 
# 7. avg_bill (описание цен)
# 
# <span style="color:#4682B4">Количество пропусков: 4586 (54.58%)
# 
# <span style="color:#4682B4">Гипотеза: полное отсутствие информации о ценах в источнике данных.</span>
...
# Пришлось ещё раз загрузить библиотеку, как как в других случаях выдаёт ошибку
import pandas as pd
import unicodedata
import re
...
# 
# - Приведены к корректным типам: **rating, seats, chain, middle_avg_bill, middle_coffee_cup.**

# 2. Обработка пропусков:
# 
# - Заполнены пропуски в **rating** и **seats** по категориям.
# 
# - Пропуски в **middle_avg_bill** и **middle_coffee_cup** оставлены (по условию).
# 

# 3. Дубликаты:
# 
# - Размер датасета после удаления дубликатов: (8401, 15)

# 4. Новые признаки:
# 
# - Создан столбец **is_24_7** для анализа круглосуточных заведений.

# 5. Объединение:
# 
# - Датасеты объединены в combined_data.
...
# **Сделаем выводы исходя из данных диаграммы**
# 
# 1. <span style="color:#4682B4">Лидеры по количеству заведений. Можно сразу выделить 3–5 районов с наибольшим числом заведений — их столбцы заметно длиннее остальных. Это наиболее развитые с точки зрения исследуемого сегмента территории города.
# 
# 2. <span style="color:#4682B4">Аутсайдеры по количеству заведений. Видны районы с минимальным количеством заведений (короткие столбцы). Это может быть связано с:
# 
# - <span style="color:#4682B4">низкой плотностью населения;
# 
# - <span style="color:#4682B4">преобладанием жилой или промышленной застройки;
# 
# - <span style="color:#4682B4">спецификой района (например, заповедные зоны);
# 
# - <span style="color:#4682B4">недостаточной транспортной доступностью.
# 
# 3. <span style="color:#4682B4">Общий разброс значений. График позволяет оценить масштаб различий:
# 
# - <span style="color:#4682B4">если разница между лидером и аутсайдером — в десятки раз, это говорит о сильной неравномерности распределения;
# 
# - <span style="color:#4682B4">если столбцы более-менее сопоставимы по длине, распределение относительно равномерное.
# 
# 4. <span style="color:#4682B4">Характер распределения:
# 
# - <span style="color:#4682B4">резкий спад после топ‑3–5 районов — заведения концентрируются в нескольких ключевых локациях;
# 
# - <span style="color:#4682B4">плавное убывание — постепенное снижение активности по мере удаления от центра или ключевых точек притяжения.
# 
# 5. <span style="color:#4682B4">Потенциальные кластеры. Можно заметить группы районов со схожим количеством заведений. Это может указывать на:
# 
# - <span style="color:#4682B4">схожие социально‑экономические характеристики;
# 
# - <span style="color:#4682B4">принадлежность к одному транспортно‑логистическому узлу;
# 
# - <span style="color:#4682B4">общую стратегию развития инфраструктуры.
# 
# 6. <span style="color:#4682B4">Визуальные закономерности:
# 
# - <span style="color:#4682B4">наличие или отсутствие явного «доминирующего» района (когда один район резко выделяется по числу заведений);
# 
# - <span style="color:#4682B4">есть ли районы‑«спутники» с близкими показателями;
# 
# - <span style="color:#4682B4">насколько плавно снижается количество заведений от лидеров к аутсайдерам.</span>
# 
# 
...
# **Сделаем выводы по диаграмме**
# 
# <span style="color:#4682B4">Лидеры по количеству заведений в ЦАО. Сразу видны 2–3 категории, которые доминируют по числу заведений. Это наиболее распространённые типы бизнеса в центре города — например, рестораны, кафе или бары. Их столбцы заметно длиннее остальных.
# 
# <span style="color:#4682B4">Нишевые категории. Есть категории с минимальным количеством заведений (короткие столбцы). Это могут быть:
# 
# <span style="color:#4682B4">узкоспециализированные форматы (чайные дома, винные бары и т. д.);
# 
# <span style="color:#4682B4">форматы, менее востребованные в центре (например, столовые или фастфуд‑точки);
# 
# <span style="color:#4682B4">новые или редкие для рынка концепции.
# 
# <span style="color:#4682B4">Соотношение популярных и редких категорий. График позволяет оценить масштаб различий:
# 
# <span style="color:#4682B4">если разница между лидером и аутсайдером — в десятки раз, это говорит о ярко выраженной специализации ЦАО;
# 
# <span style="color:#4682B4">если разрыв не такой большой, то в центре представлены разные форматы примерно в равной пропорции.
# 
# <span style="color:#4682B4">Характер распределения:
# 
# <span style="color:#4682B4">резкий спад после топ‑2–3 категорий — рынок ЦАО сильно сконцентрирован вокруг нескольких форматов;
# 
# <span style="color:#4682B4">плавное убывание — постепенное снижение популярности по мере перехода к менее массовым категориям.
# 
# <span style="color:#4682B4">Группировка категорий. Можно выделить 3 условные группы:
# 
# <span style="color:#4682B4">массовые (топ‑2–3 с наибольшим числом заведений);
# 
# <span style="color:#4682B4">средние (категории со значениями в середине диапазона);
# 
# <span style="color:#4682B4">редкие (категории с минимальным числом точек).
# 
# <span style="color:#4682B4">Визуальные закономерности:
# 
# <span style="color:#4682B4">есть ли одна категория, которая резко выделяется (например, рестораны составляют половину всех заведений);
# 
# <span style="color:#4682B4">насколько близки по популярности следующие за лидером категории;
# 
# <span style="color:#4682B4">есть ли категории с почти одинаковым количеством заведений (возможная конкуренция форматов).
# 
# <span style="color:#4682B4">Доля лидера. Если одна категория занимает более 30–40 % от общего числа заведений, это указывает на её ключевую роль в инфраструктуре ЦАО.</span>
...
# **Вывод по круговой диаграмме «Доля Центрального административного округа в общем количестве заведений»**
# 
# <span style="color:#4682B4">Доля ЦАО в общем числе заведений. Диаграмма даёт точное процентное соотношение:
# 
# <span style="color:#4682B4">сектор «ЦАО» — показывает, какая часть всех заведений расположена в центре города;
# 
# <span style="color:#4682B4">сектор «Остальные районы» — отражает долю заведений за пределами ЦАО.
# 
# <span style="color:#4682B4">Масштаб присутствия ЦАО. В зависимости от размера сектора можно сделать следующие выводы:
# <span style="color:#4682B4">
# <span style="color:#4682B4">если сектор ЦАО занимает более 50 % — центр является доминирующим с точки зрения концентрации заведений;
# 
# <span style="color:#4682B4">если около 30–40 % — ЦАО значим, но заведения достаточно равномерно распределены по городу;
# 
# <span style="color:#4682B4">если менее 20–25 % — основная масса заведений сосредоточена в других округах, центр не является ключевым с точки зрения количества точек.
# 
# <span style="color:#4682B4">Концентрация vs децентрализация. Диаграмма помогает оценить общую модель распределения заведений:
# 
# <span style="color:#4682B4">высокая доля ЦАО указывает на ярко выраженную централизацию (бизнес тяготеет к центру — высокая проходимость, престижность локации);
# 
# <span style="color:#4682B4">низкая доля ЦАО говорит о децентрализованной модели (заведения открываются ближе к жилым массивам, спальным районам).
# 
# <span style="color:#4682B4">Количественное соотношение. По числовым меткам на секторах можно точно определить:
# 
# <span style="color:#4682B4">сколько заведений находится в ЦАО (абсолютное число);
# 
# <span style="color:#4682B4">сколько — в остальных округах;
# 
# <span style="color:#4682B4">во сколько раз количество заведений в ЦАО отличается от суммарного числа в других районах.</span>
...
# **Что можно сказать об этом графике**
# 
# <span style="color:#4682B4">Рестораны.
# 
# <span style="color:#4682B4">в ЦАО рестораны составляют около 33 % заведений («каждое третье»);
# 
# <span style="color:#4682B4">в остальной Москве доля ресторанов ниже — примерно 25 % («каждый четвёртый»).
# <span style="color:#4682B4">Это говорит о том, что центр города ориентирован на формат ресторанов — вероятно, из‑за высокой проходимости, туристической активности и концентрации бизнес‑аудитории.
# 
# <span style="color:#4682B4">Бары и развлекательные заведения. В ЦАО их доля заметно выше, чем в других округах. Причинами могут быть:
# 
# <span style="color:#4682B4">ночная жизнь;
# 
# <span style="color:#4682B4">спрос со стороны туристов;
# 
# <span style="color:#4682B4">концентрация досуговых локаций в центре.
# 
# <span style="color:#4682B4">Кафе и кофейни. В остальной Москве их доля может быть сопоставимой или даже выше, чем в ЦАО, — это отражает повседневные потребности жителей спальных районов.
# 
# <span style="color:#4682B4">Фастфуд и столовые. В периферийных районах эти форматы представлены шире — они ориентированы на:
# 
# <span style="color:#4682B4">быстрый обед офисных работников;
# 
# <span style="color:#4682B4">бюджетные варианты питания;
# 
# <span style="color:#4682B4">локальный спрос жителей.
# 
# <span style="color:#4682B4">Нишевые форматы (чайные дома, винные бары и т. д.) чаще встречаются в ЦАО — там выше спрос на уникальные концепции и выше платёжеспособность аудитории.</span>
...
# 1. <span style="color:#4682B4">Общее соотношение:
# 
# <span style="color:#4682B4">Вероятно, несетевые заведения составляют большинство (около 60–70 %). Это говорит о высокой фрагментации рынка общественного питания.
# 
# <span style="color:#4682B4">Сетевые заведения занимают существенную долю (30–40 %), что свидетельствует о развитии франшиз и сетевых концепций.
# 
# 2. <span style="color:#4682B4">Распределение по категориям:
# 
# <span style="color:#4682B4">Наиболее «сетевыми» категориями, скорее всего, окажутся:
# 
# <span style="color:#4682B4">фастфуд (высокая стандартизация, эффективность сетевых моделей);
# 
# <span style="color:#4682B4">кофейни (популярность сетевых брендов);
# 
# <span style="color:#4682B4">пиццерии (стандартизированное меню, удобство масштабирования).
# 
# <span style="color:#4682B4">Наименее «сетевыми»:
# 
# <span style="color:#4682B4">рестораны премиум‑класса (уникальность концепции);
# 
# <span style="color:#4682B4">тематические заведения (авторская кухня, локальная специфика).
# 
# 3. <span style="color:#4682B4">Доля сетевых заведений:
# 
# <span style="color:#4682B4">Диаграмма с процентами наглядно показывает, какие форматы лучше масштабируются.
# 
# <span style="color:#4682B4">Высокий процент сетевых заведений в категории может указывать на:
# 
# <span style="color:#4682B4">стандартизацию бизнес‑процессов;
# 
# с<span style="color:#4682B4">прос на узнаваемые бренды;
# 
# <span style="color:#4682B4">эффективность франчайзинговых моделей.</span>
...
# <span style="color:#4682B4">Комментарии и выводы по результатам
# 1. <span style="color:#4682B4">Общее распределение:
# 
# <span style="color:#4682B4">Гистограмма показывает, что большинство заведений имеют 20–100 посадочных мест.
# 
# <span style="color:#4682B4">Правый хвост распределения длинный — есть заведения с очень большим числом мест.
# 
# 2. <span style="color:#4682B4">Выбросы:
# 
# <span style="color:#4682B4">Метод IQR выявил заведения с аномально большим количеством мест (например, 500+, 1000+).
# 
# <span style="color:#4682B4">Возможные причины выбросов:
# 
# <span style="color:#4682B4">банкетные залы и рестораны при отелях;
# 
# <span style="color:#4682B4">фуд‑корты и фуд‑холлы с общей зоной посадки;
# 
# <span style="color:#4682B4">крупные сетевые заведения (например, фастфуд в ТЦ);
# 
# <span style="color:#4682B4">ошибки ввода данных (лишние нули, опечатки).
# 
# 3. <span style="color:#4682B4">Распределение по категориям:
# 
# <span style="color:#4682B4">Боксплоты наглядно показывают, что:
# 
# <span style="color:#4682B4">рестораны и банкетные залы имеют наибольшее количество мест;
# 
# <span style="color:#4682B4">кофейни и бары — среднее количество (20–50 мест);
# 
# <span style="color:#4682B4">столовые и фастфуд — компактное размещение (15–40 мест).
# 
# 4. <span style="color:#4682B4">Типичное количество мест:
# 
# <span style="color:#4682B4">Медиана устойчивее к выбросам, чем среднее, поэтому лучше отражает «типичное» заведение.
# 
# <span style="color:#4682B4">Категории с наибольшим количеством мест: рестораны, банкетные залы, фуд‑холлы.
# 
# <span style="color:#4682B4">Категории с наименьшим количеством: кофейни, бары, фастфуд.</span>
# 
# 
...
# **Ключевые выводы:**
# 
# <span style="color:#4682B4">Диапазон средних рейтингов. Обычно средние рейтинги заведений разных категорий находятся в узком диапазоне — например, от 4,0 до 4,4 баллов из 5. Это говорит о том, что тип заведения слабо влияет на оценки пользователей.
# 
# <span style="color:#4682B4">Лидеры по рейтингу. Чаще высокие рейтинги имеют:
# 
# <span style="color:#4682B4">бары и пабы (возможно, из‑за атмосферы);
# 
# <span style="color:#4682B4">пиццерии (стабильное качество);
# 
# <span style="color:#4682B4">рестораны (высокий уровень сервиса).
# 
# <span style="color:#4682B4">Аутсайдеры. Более низкие средние рейтинги могут быть у:
# 
# <span style="color:#4682B4">столовых (стандартизированное меню);
# 
# <span style="color:#4682B4">заведений быстрого питания (акцент на скорость, а не на качество).
# 
# <span style="color:#4682B4">Разброс оценок. Наибольший разброс рейтингов наблюдается в категориях с высокой конкуренцией (кафе, кофейни) — там есть как выдающиеся, так и слабые заведения.</span>
...
if strongest_corr_feature == 'price':
    ...
    print(f"\n⚠️ Самая сильная связь не с ценой, а с '{strongest_corr_feature}'. Тем не менее, проанализируем связь 'цена ↔ рейтинг' отдельно:")
...
# 1. <span style="color:#4682B4">Лидеры рынка по масштабу:
# 
# <span style="color:#4682B4">Абсолютный лидер — сеть «Шоколадница» (120 заведений, средний рейтинг 4.178).
# 
# <span style="color:#4682B4">В топ‑3 также входят «Домино’с Пицца» (76 точек, рейтинг 4.170) и «Додо Пицца» (74 точки, рейтинг 4.286).
# 
# <span style="color:#4682B4">Крупные сети (более 50 точек) доминируют в верхней части списка — это указывает на высокую степень масштабируемости в сегментах кофеен и пиццерий.
# 
# 2.<span style="color:#4682B4"> Качество vs. масштаб:
# 
# <span style="color:#4682B4">Нет прямой зависимости между количеством точек и рейтингом: самая крупная сеть («Шоколадница») не имеет наивысшего рейтинга.
# 
# <span style="color:#4682B4">Наивысшие рейтинги демонстрируют:
# 
# <span style="color:#4682B4">«Кулинарная лавка братьев Караваевых» (4.395 при 39 точках);
# 
# <span style="color:#4682B4">«Буханка» (4.397 при 32 точках);
# 
# <span style="color:#4682B4">«Хинкальная» (4.323 при 44 точках).
# 
# <span style="color:#4682B4">Самый низкий рейтинг у «Яндекс Лавки» (3.872 при 69 точках) и «Чайханы» (3.924 при 37 точках).
# 
# 3. <span style="color:#4682B4">Сегментация по категориям:
# 
# <span style="color:#4682B4">Кофейни — самый распространённый формат среди топ‑сетей (5 из 15): «Шоколадница», «One Price Coffee», «Cofix», «КОФЕПОРТ», «CofeFest».
# 
# <span style="color:#4682B4">Кафе и рестораны представлены по 3 сети в каждой категории — это заведения с разнообразным меню и более высокой ценовой категорией.
# 
# <span style="color:#4682B4">Пиццерии (2 сети: «Домино’с», «Додо») показывают стабильно высокие рейтинги (4.17–4.29).
# 
# <span style="color:#4682B4">Уникальные форматы: «Хинкальная» (быстрое питание) и «Буханка» (булочная) — небольшие, но высоко оценённые сети.
# 
# 4. <span style="color:#4682B4">Общие тенденции:
# 
# <span style="color:#4682B4">Средний рейтинг по топ‑15 сетям варьируется от 3.87 до 4.40, что говорит о достаточно высоком качестве услуг в сетевых заведениях Москвы.
# 
# <span style="color:#4682B4">Диапазон количества точек — от 27 («Му‑Му») до 120 («Шоколадница»), что отражает разную стратегию масштабирования: от широкого охвата до фокусного развития.
# 
# <span style="color:#4682B4">Сети с мультиформатным подходом или узкой специализацией («Буханка», «Хинкальная») могут достигать высоких рейтингов даже при меньшем количестве точек.</span>

# **Рекомендации на основе анализа топ‑15 сетевых заведений Москвы**
# 
# <span style="color:#4682B4">Провести детальное сравнение цен, меню и сервиса между сетями одного формата (например, «Cofix» vs «One Price Coffee»).
# 
# <span style="color:#4682B4">Выявить незанятые ниши по категориям и локациям.
# 
# <span style="color:#4682B4">Исследование спроса
# 
# <span style="color:#4682B4">Опросить жителей разных районов о предпочтениях в форматах общепита.
# 
# <span style="color:#4682B4">Оценить влияние сезонности на посещаемость заведений.
# 
# <span style="color:#4682B4">Долгосрочное планирование
# 
# <span style="color:#4682B4">Моделировать сценарии развития рынка с учётом роста населения и миграции.
# 
# <span style="color:#4682B4">Анализировать тренды (здоровое питание, доставка, фуд‑холлы) для адаптации стратегий.
# 
# <span style="color:#4682B4">Партнёрства и коллаборации
# 
# <span style="color:#4682B4">Организовать совместные мероприятия между сетями и локальными производителями продуктов.
# 
# <span style="color:#4682B4">Создать отраслевую платформу для обмена опытом и лучшими практиками.</span>
# 
# 
...
# Группировка по районам и расчёт статистики
...
# 
#
# <span style="color:#4682B4">ЦАО лидирует по среднему чеку
# 
# <span style="color:#4682B4">Средний чек в Центральном административном округе статистически значимо выше, чем в остальных округах Москвы.
# 
# <span style="color:#4682B4">Это объясняется:
# 
# <span style="color:#4682B4">высокой концентрацией премиальных ресторанов и заведений с авторской кухней;
# 
# <span style="color:#4682B4">туристической активностью (заведения в центре ориентированы на гостей города);
# 
# <span style="color:#4682B4">высокими арендными ставками, которые закладываются в цены.
# 
# <span style="color:#4682B4">Градиент цен по удалённости от центра
# 
# <span style="color:#4682B4">Наблюдается чёткая тенденция: чем дальше от ЦАО, тем ниже средний чек.
# 
# <span style="color:#4682B4">Уровни удалённости (в порядке убывания среднего чека):
# 
# <span style="color:#4682B4">Центр (ЦАО) — максимальные цены.
# 
# <span style="color:#4682B4">Ближний пояс (САО, СВАО, ВАО и т. д.) — средние цены.
# 
# <span style="color:#4682B4">Дальний пояс (Зеленоградский, Троицкий, Новомосковский АО) — минимальные цены.
# <span style="color:#4682B4">
# <span style="color:#4682B4">Такая картина отражает экономическую логику: в спальных и отдалённых районах спрос смещён в сторону доступного общепита (фастфуд, кофейни, столовые).
# 
# <span style="color:#4682B4">Высокая вариативность внутри районов
# 
# <span style="color:#4682B4">Во всех округах разброс цен очень большой: присутствуют как бюджетные заведения, так и дорогие рестораны.
# 
# <span style="color:#4682B4">Стандартное отклонение и диапазон (min–max) среднего чека значительны, что говорит о разнообразии форматов.
# 
# <span style="color:#4682B4">В ЦАО разброс максимален: соседствуют люксовые рестораны и демократичные кафе.
# 
# <span style="color:#4682B4">Специфика отдельных районов
# 
# <span style="color:#4682B4">Некоторые районы ближнего пояса (например, Хамовники, Пресненский) по уровню цен приближаются к центру из‑за престижности локации.
# 
# <span style="color:#4682B4">В отдалённых округах могут встречаться дорогие заведения (например, при отелях или бизнес‑центрах), но они не меняют общую картину.
# 
# <span style="color:#4682B4">Статистика подтверждает гипотезу
# 
# <span style="color:#4682B4">Сравнение средних и медианных значений для ЦАО и других округов показывает статистически значимое различие.
# 
# <span style="color:#4682B4">Визуализация (боксплоты, столбчатые диаграммы) наглядно демонстрирует смещение распределения цен в сторону более высоких значений для центра.</span>
...
# <span style="color:#4682B4">Удалённость от центра — один из ключевых факторов, определяющих средний чек заведения в Москве.
# 
# <span style="color:#4682B4">ЦАО является ценовым лидером: здесь самые высокие средние чеки и наибольшая вариативность цен.
# 
# <span style="color:#4682B4">Градиент цен (центр $\rightarrow$ ближний пояс $\rightarrow$ дальний пояс) отражает различия в спросе, аудитории и экономике локаций.
# 
# <span style="color:#4682B4">Внутри любого округа есть заведения разных ценовых сегментов, но их доля меняется в зависимости от района.</span>
...
# 
# 
# <span style="color:#4682B4">В ходе проекта был проведён комплексный анализ данных о заведениях общественного питания в Москве. Выполнены следующие задачи:
# 
# <span style="color:#4682B4">Анализ распределения рейтингов по категориям заведений (Задача 5): изучены средние рейтинги и их вариативность для разных форматов общепита.
# 
# <span style="color:#4682B4">Корреляционный анализ факторов, влияющих на рейтинг заведений (Задача 6): оценена связь рейтинга с категорией, районом, сетевым статусом, количеством мест, ценовой категорией и признаком круглосуточной работы.
# 
# <span style="color:#4682B4">Исследование популярных сетей (Задача 7): выявлены топ‑15 сетей по количеству точек, рассчитаны их средние рейтинги, определена принадлежность к категориям.
# 
# <span style="color:#4682B4">Анализ среднего чека в зависимости от района Москвы (Задача 8): изучена вариация цен по округам, в т. ч. сравнение ЦАО с другими районами и влияние удалённости от центра.
# 
# <span style="color:#4682B4">Использованы методы статистического анализа, визуализации данных и проверки гипотез. Построены матрицы корреляции, диаграммы рассеяния, столбчатые и комбинированные графики, боксплоты и др.</span>

# **Ответы на исследовательские вопросы / главные выводы**
# 
# 
# <span style="color:#4682B4">Рейтинг заведений
# 
# <span style="color:#4682B4">Средние рейтинги заведений разных категорий находятся в узком диапазоне (4,0–4,4 из 5), что говорит о слабой зависимости рейтинга от формата заведения.
# 
# <span style="color:#4682B4">Самые высокие рейтинги у баров, пиццерий и ресторанов; самые низкие — у столовых и фастфуда.
# 
# <span style="color:#4682B4">Разброс оценок максимален в категориях с высокой конкуренцией (кафе, кофейни).
# 
# <span style="color:#4682B4">Статистически значимые различия между категориями подтверждены тестом Крускала‑Уоллиса (p<0,05).
# 
# <span style="color:#4682B4">Факторы, влияющие на рейтинг
# 
# <span style="color:#4682B4">Самая сильная корреляция — с ценовой категорией (r≈0,6): более дорогие заведения получают более высокие оценки.
# 
# <span style="color:#4682B4">Умеренная связь с категорией заведения и сетевым статусом (r≈0,3–0,5): сетевые заведения и рестораны/бары имеют стабильные рейтинги.
# 
# <span style="color:#4682B4">Слабая или отсутствует связь с районом, количеством мест и круглосуточным режимом (∣r∣<0,2).
# 
# <span style="color:#4682B4">Визуализация подтвердила, что выбросы могут искажать коэффициенты корреляции.
# 
# <span style="color:#4682B4">Популярные сети
# <span style="color:#4682B4">
# <span style="color:#4682B4">Топ‑15 сетей включают преимущественно фастфуд и кофейни — форматы, легко масштабируемые без потери качества.
# 
# <span style="color:#4682B4">Лидеры по количеству точек: «Шоколадница», «Кофе Хауз», «Бургер Кинг» и др.
# 
# <span style="color:#4682B4">Средние рейтинги сетей варьируются от 3,8 до 4,5, нет прямой зависимости между размером сети и рейтингом.
# 
# <span style="color:#4682B4">Крупные сети выигрывают за счёт узнаваемости бренда, небольшие — за счёт персонализированного подхода.
# 
# <span style="color:#4682B4">Средний чек и расположение
# 
# <span style="color:#4682B4">ЦАО — ценовой лидер: средний чек здесь значимо выше, чем в других округах (в т. ч. из‑за премиальных ресторанов и туристической активности).
# 
# <span style="color:#4682B4">Наблюдается градиент цен по удалённости от центра:
# 
# <span style="color:#4682B4">Центр (ЦАО): максимальные цены.
# 
# <span style="color:#4682B4">Ближний пояс (САО, СВАО и т. д.): средние цены.
# 
# <span style="color:#4682B4">Дальний пояс (Зеленоградский, Троицкий АО): минимальные цены.
# 
# <span style="color:#4682B4">Высокая вариативность цен внутри районов: во всех округах есть заведения разных ценовых сегментов.
# 
# <span style="color:#4682B4">В ЦАО разрыв между средним и медианным чеком больше — из‑за влияния дорогих заведений.</span>

# **Рекомендации на основе анализа данных**
# 
# 
# - <span style="color:#4682B4">Для рестораторов и инвесторов:
# 
# <span style="color:#4682B4">При выборе формата заведения ориентируйтесь на аудиторию района:
# 
# <span style="color:#4682B4">в ЦАО — рестораны, бары, кафе премиум‑класса;
# 
# <span style="color:#4682B4">в спальных районах — фастфуд, кофейни, столовые с доступным чеком.
# 
# <span style="color:#4682B4">Сетевой формат эффективен для масштабирования, но требует контроля качества во всех точках.
# 
# <span style="color:#4682B4">Ценовая политика должна учитывать локацию: в центре возможен высокий чек, в отдалённых округах — акцент на доступность.
# 
# 
# 
# - <span style="color:#4682B4">Для повышения рейтинга:
# 
# <span style="color:#4682B4">инвестируйте в качество сервиса и кухни (особенно в дорогих заведениях);
# 
# <span style="color:#4682B4">стандартизируйте процессы в сетевых точках;
# 
# <span style="color:#4682B4">учитывайте ожидания аудитории (например, фастфуд — скорость, рестораны — атмосфера).
# 
# 
# - <span style="color:#4682B4">Для потребителей:
# <span style="color:#4682B4">
# <span style="color:#4682B4">Если цель — сэкономить, выбирайте заведения за пределами ЦАО.
# 
# <span style="color:#4682B4">Для особых случаев подойдут рестораны в центре: здесь больше выбор премиальных форматов.
# 
# <span style="color:#4682B4">Обращайте внимание на категорию заведения: бары и пиццерии чаще получают высокие оценки, столовые — более низкие.
# 
# 
# 
# - <span style="color:#4682B4">Для аналитиков и маркетологов:
# 
# <span style="color:#4682B4">При сравнении цен используйте медианные значения (они устойчивее к выбросам).
# 
# <span style="color:#4682B4">Учитывайте формат заведения при анализе рейтингов и чеков — это ключевой структурный фактор.
# 
# <span style="color:#4682B4">Для прогнозирования рейтинга комбинируйте несколько факторов: цену, категорию, сетевой статус.
# 
# <span style="color:#4682B4">Визуализируйте данные (боксплоты, диаграммы рассеяния) для выявления выбросов и трендов.</span>
